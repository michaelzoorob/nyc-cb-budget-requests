...
- **Use 311 or another channel.** A response says this is not a budget request, or points to 311, a service request, an online form, a permit or program application, or another agency or process that handles it.
...
For each record return `id`, `action`, `purpose` and `why`, plus `contact` and `url` when they apply.
